...
import { saveDemoProduct } from '@/lib/demoProductsStore';
import {
  ArrowLeft,
  Upload,
  X,
...
export default function AddProductPage() {
  ...
  const [department, setDepartment] = useState<DepartmentType>('Sarees');
  const [categorySlug, setCategorySlug] = useState<string>('printed-cotton');
  ...
  const [blousePieceIncluded, setBlousePieceIncluded] = useState(true);
  const [workTechnique, setWorkTechnique] = useState('Traditional Handloom Weave');
  const [color, setColor] = useState('');
  const [occasion, setOccasion] = useState(SAREE_OCCASIONS[0]);

  // Ladies Suits
  const [suitType, setSuitType] = useState<'Full Set' | 'Separate Pieces'>('Full Set');
  const [suitSize, setSuitSize] = useState(SUIT_SIZES[0]);

  // Bed Sheets
  const [bedSize, setBedSize] = useState(BED_SIZES[3]); // King by default
  const [pillowCoversIncluded, setPillowCoversIncluded] = useState(true);

  // Form Validation & Feedback
  const [formError, setFormError] = useState('');
  const [isSubmitting, setIsSubmitting] = useState(false);

  // When department changes, sync categories and default fabric
  useEffect(() => {
  ...
  // When category changes, update suggested fabric if not modified
  ...
  // Image Upload Handling (Local File Reader for demo preview)
  ...
  // Preset Sample Images for Quick Testing by Business Owner
  ...
  // Submit Handler
  const handleSubmit = (e: React.FormEvent, overrideStatus?: ProductStatusType) => {
    e.preventDefault();
    setFormError('');
    ...
    // Fallback default image if none uploaded in prototype
    ...
    <div className="space-y-8 max-w-5xl mx-auto pb-16">
      ...
      <div>
        <div className="flex items-center gap-2 text-xs text-gray-400 mb-2">
          ...
          <span>/</span>
          <span className="text-[#FAF7F2]">Add New Product</span>
        </div>
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 pb-6 border-b border-white/10">
          <div>
            ...
            <p className="text-xs text-gray-400 mt-1">
              Create product listings with department-tailored specifications for review.
            </p>
          </div>
          ...
          <div className="flex items-center gap-2.5">
            ...
            <button
              ...
              className="px-4 py-2.5 rounded-xl bg-[#251D20] hover:bg-[#2F2529] border border-[#D4AF37]/30 text-xs text-[#D4AF37] font-medium transition-colors"
            >
              Save as Draft
            </button>
            <button
              ...
              disabled={isSubmitting}
              className="px-5 py-2.5 rounded-xl bg-[#D4AF37] hover:bg-[#B8952B] text-[#1A1315] text-xs font-semibold uppercase tracking-wider transition-all shadow-md flex items-center gap-1.5"
            >
      ...
      {/* UX Prototype Information Banner */}
      <div className="p-4 rounded-2xl bg-[#1E181A] border border-[#D4AF37]/30 flex items-start gap-3 text-xs">
        ...
        <div className="space-y-1">
          <span className="font-semibold text-[#D4AF37] uppercase tracking-wider text-[11px] block">
            Phase B Design & Attribute Review Mode
          </span>
          <p className="text-gray-300 text-xs leading-relaxed">
            This interactive form is connected to an in-memory demo store to review fields, department-specific attributes, and workflow ergonomics with the store owner before finalizing the permanent database schema.
          </p>
        </div>
      </div>
      ...
      <form onSubmit={(e) => handleSubmit(e)} className="space-y-8">
        ...
        {/* =========================================================================
            SECTION 3: Product Imagery (Multi-Image Upload / Local Demo)
        ========================================================================= */}
        <div className="p-6 sm:p-8 rounded-2xl bg-[#1E181A] border border-[#D4AF37]/20 space-y-6">
          ...
          <div className="border-2 border-dashed border-[#D4AF37]/30 rounded-2xl p-6 text-center hover:border-[#D4AF37]/60 transition-colors bg-[#140F11]/50">
            ...
            >
              ...
              <p className="text-[11px] text-gray-500">Supports PNG, JPG, WEBP up to 5MB (Local prototype mode)</p>
            </label>
          </div>
          ...
          <div className="space-y-2">
            <span className="text-[11px] uppercase tracking-wider text-gray-400 block">
              Quick Prototype Presets (Click to add for testing):
            </span>
            <div className="flex flex-wrap gap-2">
              <button
                type="button"
                onClick={() => addSampleImage('/images/sarees/01_printed_cotton.jpg')}
                className="text-[11px] px-3 py-1.5 rounded-lg bg-[#251D20] text-gray-300 hover:text-[#D4AF37] border border-white/5 transition-colors"
              >
              ...
              {images.map((imgSrc, idx) => (
                ...
                >
                  ...
                  <button
                    ...
                    className="absolute top-1.5 right-1.5 w-6 h-6 rounded-full bg-black/70 text-white flex items-center justify-center opacity-90 hover:bg-red-600 transition-colors"
                    title="Remove image"
                  >
        ...
        <div className="p-6 rounded-2xl bg-[#1E181A] border border-[#D4AF37]/20 flex flex-col sm:flex-row items-center justify-between gap-4">
          <div className="text-xs text-gray-400 flex items-center gap-2">
            ...
            <span>All changes are previewed in the temporary prototype store.</span>
          </div>
          ...
          <div className="flex items-center gap-3 w-full sm:w-auto">
            ...
            <button
              ...
              onClick={(e) => handleSubmit(e, 'Draft')}
              disabled={isSubmitting}
              className="flex-1 sm:flex-none px-5 py-3 rounded-xl bg-[#251D20] hover:bg-[#2F2529] border border-[#D4AF37]/30 text-xs text-[#D4AF37] font-medium transition-colors cursor-pointer"
            >
              Save as Draft
            </button>
            <button
              ...
              onClick={(e) => handleSubmit(e, 'Active')}
              disabled={isSubmitting}
              className="flex-1 sm:flex-none px-6 py-3 rounded-xl bg-[#D4AF37] hover:bg-[#B8952B] text-[#1A1315] text-xs font-semibold uppercase tracking-wider transition-all shadow-md cursor-pointer flex items-center justify-center gap-2"
            >
              <CheckCircle className="w-4 h-4" />
              <span>{isSubmitting ? 'Saving...' : 'Publish Product'}</span>
            </button>
